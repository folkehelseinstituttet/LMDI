...
To gain access to the API for data transfer, the sender system's IP address must be registered in LMR's list of allowlisted addresses. Both individual addresses and address ranges can be allowlisted.
Allowlisting is only available in the production environment.


#### Certificates

Data transferred to the API must be encrypted and signed (see [Signed and Encrypted Bundle](en-SignertKryptertBundle.html)).
...
- Encryption of data uses the public key in the Medication Registry's enterprise certificate. This can be downloaded [here](en-nedlastinger.html).
