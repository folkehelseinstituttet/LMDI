# C# example code - Legemiddeldata fra institusjon til Legemiddelregisteret v1.1.2
...
#### 1. Creating a compressed, encrypted, and signed bundle (SignertKryptertBundle)

This class handles encryption, signing, and compression of data using X.509 certificates. It provides functionality for:

* Compressing message content with GZip
* Encrypting data with AES-GCM for secure transfer
* Encrypting AES keys with the receiver’s public RSA key
* Signing messages with the sender’s private RSA key
* Creating a combined bundle of message data with metadata, encrypted content, and signature

The class uses certificates from the Windows Certificate Store and requires thumbprints for both the sender and receiver certificates.
...
// Handles encryption, signing, and compression of message content with X.509 certificates
    ...
    string senderOrganizationIdentifier)
{
    ...
    private byte[] EncryptKey(byte[] key, string receiverThumbprint)
    {
        try
        {
            ...
            {
                throw new InvalidOperationException($"Public key not available in receiver's certificate (thumbprint: {receiverThumbprint}).");
            }
    ...
    // Gets the RSA public key from a certificate based on thumbprint
    ...
    // Gets the RSA private key from a certificate based on thumbprint
    ...
    // Gets an X.509 certificate from the certificate store based on thumbprint
